...
COCO128_ZIP = "https://github.com/ultralytics/assets/releases/download/v0.0.0/coco128.zip"
...
def download_and_extract():
    if BUILD_DIR.exists():
        print(f"COCO128 already exists at {BUILD_DIR}")
        return
...
def main():
    ...
    if len(pairs) < len(flat_splits):
        raise SystemExit(f"Not enough person images found ({len(pairs)}). Lower quotas.")
    ...
    print(f"Dataset created at {OUT_DIR} (train/val/test).")
    print(f"Train: {TARGET_COUNTS['train']} images")
    print(f"Val: {TARGET_COUNTS['val']} images")
    print(f"Test: {TARGET_COUNTS['test']} images")
